...
·Added support check (IsSupported~) and permission granted check (IsPermissionGranted) to several "~Controller"[*] at startup (Start()). When it is not possible, an error message is returned to 'OnError' callback.
[*]SpeechRecognizerDialogController, BluetoothSettingController, SpeechRecognizerController, VibratorController, HeartRateController, All other sensors (IsSupportedSensor only).
(ver.1.10)
