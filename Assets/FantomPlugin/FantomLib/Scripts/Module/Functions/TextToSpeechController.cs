...
namespace FantomLib
{
    ...
    public class TextToSpeechController : MonoBehaviour
    {
        ...
        //Inspector Settings
        [SerializeField, Range(0.5f, 2.0f)] private float speed = 1;                //Speech speed
        [SerializeField, Range(STEP_MIN, 2.0f)] private float speedStep = 0.05f;    //Speech speed increase/decrease amount
        [SerializeField, Range(0.5f, 2.0f)] private float pitch = 1;                //Voice pitch
        ...
        public bool saveSetting = false;                    //Whether to save the text (Also local value is always overwritten).
...
#region Properties and Local values Section
        ...
        private static int initialized = -1;            //1:success, 0:fail, -1:not yet
        private static string initializeStatus = "";    //initialize status message
        ...
        private void ReceiveStatus(string message)
        {
            ...
            if (initialized == -1)  //At first time
            {
...
#region Other method Section
        ...
        {
            int k = multiple.ToString().Length - 2;     //Number of digits after the decimal point. (ex) "0.05" -> "0." (Integer part = 1+'.'=2) + "05" (Decimal part = 4-2=2) 
            int p = pow10[k];                           //10^k
            ...
            float f = Mathf.Floor((float)v / m) * m;    //If calculate it as float, get an error here. (ex) Mathf.Floor(1.05f / 0.05f)=20 -> Mathf.Floor(105f / 5)=21
